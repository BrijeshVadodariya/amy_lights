...
const PurchaseDetail = ({ purchaseId, onBack, onNavigate }) => {
  ...
  return (
    <>
      <div className="delivery-detail-page">
        ...
        <div className="delivery-navbar">
          ...
          <div className="flex items-center gap-2">
            <button 
              className="btn-ui h-[38px] px-3 md:px-4 font-bold border-slate-200 hover:bg-slate-50" 
            ...
            <button 
              className="btn-ui h-[38px] px-3 md:px-4 font-bold border-emerald-100 text-emerald-600 hover:bg-emerald-50" 
          ...
          <div className="delivery-status-bar">
            ...
            <div className="status-steps-container">
              {['draft', 'sent', 'to_approve', 'purchase', 'done'].map((st) => {
                ...
                return (
                  <div key={st} className={`status-step ${isActive ? 'active' : ''}`}>
                    <span>{labels[st]}</span>
                  </div>
                );
              })}
      ...
      {/* Mobile Action Bar */}
      <div className="md:hidden fixed bottom-6 right-6 flex flex-col gap-3 z-[100]">
        <button 
          className="w-12 h-12 rounded-full bg-white shadow-lg border border-slate-200 flex items-center justify-center text-slate-600"
          onClick={() => odooService.printPurchase(po.id)}
        >
          <Printer size={20} />
        </button>
        ...
        <button 
          className="w-14 h-14 rounded-full bg-blue-600 shadow-xl flex items-center justify-center text-white"
          onClick={() => onNavigate('create-purchase', po.id)}
        >
          <Pencil size={24} />
        </button>
