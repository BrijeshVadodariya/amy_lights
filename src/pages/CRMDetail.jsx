...
const CRMDetail = ({ leadId, onBack, onNavigate }) => {
  ...
  const handleConvertToOrder = (targetState) => {
    ...
    onNavigate(route, null, { 
      lead: lead, 
      targetState: targetState,
      returnRoute: 'crm-detail'
    });
  };
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                    ...
                    {lead.user_phone && (
                      <span style={{ fontSize: '11px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '3px', marginTop: '2px' }}>
                        <Phone size={10} className="text-blue-500" /> {lead.user_phone}
                      </span>
                    )}
                  </div>
                </div>
                <div style={{ borderTop: '1px dashed #e2e8f0', paddingTop: '8px', marginTop: '4px' }}>
                  <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '4px' }}>{lead.contact_name || (Array.isArray(lead.partner_id) ? lead.partner_id[1] : '—')}</div>
            ...
            <div style={{ padding: '14px', background: '#fff' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                {(lead.other_name || lead.other_number) && (
                  <div className="compact-pro-row" style={{ background: '#f3f4f6', padding: '8px', borderRadius: '6px' }}>
                    ...
                    {lead.other_number && (
                      <div style={{ fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                        <Phone size={10} className="text-blue-500" /> {lead.other_number}
                      </div>
                    )}
                  </div>
                )}
